...
class TransformerScheduler(BaseScheduler):
    ...
    def get_step_lr(self, step):
        #print(self.factor * self.model_size ** (-0.5) * min(step ** (-0.5), step * self.warmup_steps ** (-1.5)))
        return self.factor * self.model_size ** (-0.5) * min(step ** (-0.5), step * self.warmup_steps ** (-1.5)) #* 10#00


class ConformerScheduler(BaseScheduler):
    def __init__(self, optimizer, warmup_steps):
        self.optimizer_lr = optimizer.state_dict()['param_groups'][0]['lr']
        self.warmup_steps = warmup_steps
        super(ConformerScheduler, self).__init__(optimizer, stepwise=True)

    def get_step_lr(self, step):
        return self.optimizer_lr * self.warmup_steps ** 0.5 * min(step ** -0.5, step * self.warmup_steps ** -1.5)
