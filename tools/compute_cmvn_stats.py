...
class CollateFunc(object):
    ...
    '''
    def __init__(self, feat_dim, resample_rate, scale_factor):
    ...
    def __call__(self, batch):
        ...
        for item in batch:
            ...
            waveform = waveform * (1 << self.scale_factor)
            ...
            number += mat.shape[0]
        #     logger.info('complete a item with {} seconds'.format(time.time() - tsi))
        # logger.info('complete a batch with {} seconds'.format(time.time() - tsb))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale_factor',
                        default=0,
                        type=int,
                        help='scaling factor used to scale wav')
    ...
    feat_dim = configs['data']['fbank_conf']['num_mel_bins']
    resample_rate = 0
    if 'resample_conf' in configs['data']:
        resample_rate = configs['data']['resample_conf']['resample_rate']
        logger.info('using resample and new sample rate is {}'.format(resample_rate))
        # print('using resample and new sample rate is {}'.format(resample_rate))

    collate_func = CollateFunc(feat_dim, resample_rate, args.scale_factor)
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(data_loader):
            ...
            if wav_number % 1000 == 0:
                logger.info(f'processed {wav_number} wavs, {all_number} frames')
